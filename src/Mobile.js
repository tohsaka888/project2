...
const Mobile = () => {
  ...
  const logout = async () => {
    const res = await fetch(
      `http://musicapi.leanapp.cn/logout?cookie=${localStorage.neteaseCookie}`,
      {
        mode: "cors",
        credentials: "include",
      }
  ...
  useEffect(() => {
    ...
    const isLogin = async () => {
      const res = await fetch(
        `http://139.196.141.233:3000/login/status?cookie=${localStorage.neteaseCookie}`,
        {
          mode: "cors",
          credentials: "include",
        }
            ...
            <div style={{ height: "50px", background: "red" }}>
              <Affix offsetTop={0}>
                <NavBar
                  ...
                  leftContent={[<MenuOutlined key={"0"} />]}
                  rightContent={[
                    <Icon
                      key="1"
                      type="search"
              ...
              >
                {loginStatus.code === 301 && (
                  <div style={{ marginTop: "24px" }}>
                    ...
                    <Flex justify={"center"}>
                      <span style={{ fontFamily: "text", fontSize: "30px" }}>
                        游客账户
                      </span>
                    </Flex>
                  </div>
