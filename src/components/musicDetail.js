...
import {PlayCircleOutlined,EnvironmentOutlined,LikeOutlined,DislikeOutlined} from '@ant-design/icons'
...
const MusicDetail = ({setMusicUrl,loginStatus,cookie}) => {
    ...
    return (
        <Content style={{marginLeft:"15vw",marginRight:"15vw",background:"white"}}>
                            ...
                            if(change.state>=item.start && change.state<=item.end) {
                                return (
                                    <Paragraph key={index} style={{float:"left",width:"26.3vw"}}>
                                        <span style={{float:"left",color:"red",fontWeight:"bolder"}} ref={color}>{item.text}</span>
                                    </Paragraph>
                                )
                            }else{
                                return (
                                    <Paragraph key={index} style={{float:"left",width:"26.3vw"}}>
                                        <span style={{float:"left"}} ref={color}>{item.text}</span>
                                    </Paragraph>
                                )
                            }
                        })}
            ...
            <Comment style={{width: "90%", float: "left",marginLeft:"5%"}}
                     content={
                         <div>
                             <TextArea style={{
                                 marginTop: "20px",
